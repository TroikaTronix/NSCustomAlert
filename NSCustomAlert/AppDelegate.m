...
static BOOL appearanceIsDark(NSAppearance * appearance)
{
    if (@available(macOS 10.14, *)) {
        NSAppearanceName basicAppearance = [appearance
...
- (void)applicationDidFinishLaunching:(NSNotification*)aNotification {
  if (@available(macOS 10.14, *)) {
    ...
    NSString* appearanceMode = [[NSUserDefaults standardUserDefaults]
        stringForKey:@"AppleInterfaceStyle"];

    if (appearanceMode != nil &&
        ([appearanceMode compare:@"Dark"] == NSOrderedSame)) {
...
- (IBAction)showCustomAlertShort:(id)senderId {
  NSString* messageText =
      @"This is a shorter error talking about something that went wrong. ";

  NSString* informativeText =
      @"This is the informative text that will be longer but not so long as to "
...
- (IBAction)showCustomBelowEdgeCase:(id)senderId {
  NSString* messageText =
      @"This is a shorter error talking about something that went wrong. We "
      @"will keep extending to the edge case.";

  NSString* informativeText =
      @"This is the informative text that will be longer but not so long as to "
      @"trigger the custom alert. Let's add some more text to see what's how "
      @"far we can go before we cross the threshold into the next line.";
...
- (IBAction)showCustomAboveEdgeCase:(id)senderId {
  NSString* messageText =
      @"This is a shorter error talking about something that went wrong. We "
      @"will keep extending it until it reaches the edge case.";

  NSString* informativeText =
      @"This is the informative text that will be longer but not so long as to "
      @"trigger the custom alert. Let's add some more text to see what's how "
      @"far we can go before we cross the threshold into the next line, which "
      @"is here.";
...
- (void)showCustomAlertForText:(NSString*)inMsgText
                      infoText:(NSString*)inInfoText {
  id<NSCustomAlertProtocol> alertp =
      [NSCustomAlert createAlertForMessageText:inMsgText infoText:inInfoText];
  ...
  // alertp.accessoryView = [[[NSView alloc] initWithFrame:NSMakeRect(0, 0, 500,
  // 0)] autorelease];
  ...
  [alertp beginSheetModalForWindow:_window
                 completionHandler:^(NSInteger response) {
                   [alertp release];
                   NSLog(@"Response = %d\n", (int)response);
                 }];
